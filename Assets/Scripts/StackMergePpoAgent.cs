...
namespace StackMerge
{
    ...
    public sealed class StackMergePpoAgent
    {
        ...
        private const float ProgressUpdateScale = 650f;

        private readonly StackMergePpoTrainingData data;
        private readonly List<Transition> trajectory = new(192);
        ...
        public float TrainingProgress => Clamp01(1f - (float)Math.Exp(-Math.Max(0, data.updates) / ProgressUpdateScale));
        ...
        public void Observe(MoveResult result, StackMergeGameState nextState, bool trainingMode)
        {
            ...
            int targetBatch = trainingMode ? 48 : 112;
        ...
        public string BuildStatus(long bestScore, int bestHigh, bool trainingMode)
        {
            ...
            return $"PPO Lv {Level} | Updates {metrics.Updates} | Steps {metrics.Steps} | Avg reward {metrics.RecentAverageReward:0.00} | Policy loss {metrics.LastPolicyLoss:0.000} | Value loss {metrics.LastValueLoss:0.000} | Entropy {metrics.LastEntropy:0.000} | Best {bestScore} / {bestHigh} | Mode {(trainingMode ? "Training: no chips, faster PPO updates" : "Normal: earns chips, slower PPO updates")}";
        }
        ...
        private void CompleteEpisode(StackMergeGameState state)
        {
            ...
            data.recentAverageReward = data.episodes <= 1
                ? episodeReward
                : data.recentAverageReward * 0.92f + episodeReward * 0.08f;
        ...
        private void UpdatePolicy(bool trainingMode)
        {
            ...
            int epochs = trainingMode ? 4 : 1;
            float actorLearningRate = trainingMode ? 0.00125f : 0.00028f;
            float criticLearningRate = trainingMode ? 0.00175f : 0.00035f;
            float entropyCoefficient = trainingMode ? 0.014f : 0.004f;
        ...
        private float TrainActor(Transition transition, float advantage, float learningRate, float entropyCoefficient, out float entropy)
        {
            ...
            float coefficient = clipped ? 0f : advantage * ratio;
            ...
            for (int hiddenIndex = 0; hiddenIndex < HiddenSize; hiddenIndex++)
            {
                float gradient = hiddenGradient[hiddenIndex] * (1f - hidden[hiddenIndex] * hidden[hiddenIndex]);
                if (Math.Abs(gradient) < 1e-8f)
                {
                    continue;
                }
        ...
        private float TrainCritic(float[] features, float targetReturn, float learningRate)
        {
            ...
            float error = targetReturn - value;
            ...
            for (int hiddenIndex = 0; hiddenIndex < HiddenSize; hiddenIndex++)
            {
                float gradient = data.criticW2[hiddenIndex] * error * (1f - hidden[hiddenIndex] * hidden[hiddenIndex]);
                int weightOffset = hiddenIndex * InputSize;
                for (int inputIndex = 0; inputIndex < InputSize; inputIndex++)
                {
        ...
        {
            float exploration = trainingMode ? 1f : 0.06f;
        ...
        private static float ComputeReward(MoveResult result, StackMergeGameState nextState, long scoreBefore)
        {
            ...
            float reward = 0.02f;
            reward += (float)Math.Log(1.0 + scoreDelta, 2.0) * 0.18f;
            reward += result.MergeCount * 0.58f;
            reward += LogBlock(Math.Max(result.ResultingTopValue, result.HighestBlock)) * 0.045f;
            reward += FreeSlots(nextState) * 0.012f;
            reward += nextState.GetLegalMoveIndices().Length * 0.055f;
            reward += result.UnstableSaveUsed ? 0.20f : 0f;
            reward += result.ActionKind == SolverActionKind.QueueSkip ? -0.03f : 0f;
            reward += result.ActionKind == SolverActionKind.Pickaxe ? -0.04f : 0f;
            reward -= nextState.Stacks.Count(stack => stack.Count >= nextState.StackCapacity - 1) * 0.09f;
            reward -= result.IsGameOver ? 2.5f : 0f;
            return Math.Min(6f, Math.Max(-6f, reward));
        }

        private void EnsureInitialized(Random random)
        {
            random ??= new Random(24681357);
            bool invalid = data.version != Version
                || data.actorW1 == null
            ...
            int updates = Math.Max(0, data.updates);
            int episodes = Math.Max(0, data.episodes);
            int steps = Math.Max(0, data.steps);
            float totalReward = Math.Max(0f, data.totalReward);
            float recentAverageReward = data.recentAverageReward;
            float bestEpisodeReward = data.bestEpisodeReward;
            long bestScore = Math.Max(0, data.bestScore);
            int bestHigh = Math.Max(0, data.bestHigh);
